...
export class ApiService {
  ...
  verifyOtp(oneTimePassword: OneTimePassword,regUser : User):Observable<any> {
    const URL = `http://localhost:8080/auth/verifyotp?id=${oneTimePassword.id}&oneTimePasswordCode=${oneTimePassword.oneTimePasswordCode}&expires=${oneTimePassword.expires.toISOString()}`;
